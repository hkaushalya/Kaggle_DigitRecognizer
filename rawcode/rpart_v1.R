...
## Load data
DATASET.train <- read.csv('data/train.csv')
DATASET.test  <- read.csv('data/test.csv')
...
colnames(DATASET.train) <- c('Y', paste('X', 1:784, sep=""))
...
colnames(DATASET.test) <- c(paste('X', 1:784, sep=""))
...
str(prediction.rpart)

DATASET.test$Y <- prediction.rpart
